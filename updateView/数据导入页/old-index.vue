<template>
  <div class="app-container">
    ...
    <el-row>
      <select-factory v-model="factory"  filterable  placeholder="请选择数据导入所属厂别" @change="factoryChange"></select-factory>
      <el-button type="primary" @click="addRow" :disabled="this.factory===''">新增行</el-button>
      <el-button type="primary" @click="handleImport" :disabled="this.factory===''">导入</el-button>
      <el-button type="primary" @click="handleExport">导出</el-button>
      <el-button type="success" @click="checkInfo" >校验</el-button>
      <el-button type="primary" @click="updateInfo" >修改</el-button>
      <el-button type="primary" @click="saveInfo" :loading="isLoading"  :disabled="!isSave" v-show="!isProceed">保存</el-button>
      <el-button type="primary" @click="cleanTable">清空</el-button>
      <el-button type="success" @click="saveInfo" v-show="isProceed" >继续入库</el-button>
    ...
    <u-table
      ...
      <u-table-column prop="factory" label="厂别" width="100">
        <template slot-scope="scope">
          <el-select v-model="scope.row.factory" clearable :disabled="true">
      ...
      <u-table-column prop="analysisType" label="药水类型" width="100">
        <template slot-scope="scope">
          <el-select v-model="scope.row.analysisType" clearable :disabled="isCheck">
      ...
      <u-table-column prop="measureMachine" label="设备编号" width="100">
        <template slot-scope="scope">
          <el-input v-model="scope.row.measureMachine" :disabled="isCheck"/>
        </template>
      </u-table-column>
      <u-table-column prop="sampleReceivingTime" label="送样时间" width="100">
        <template slot-scope="scope">
          <el-date-picker
      ...
      <u-table-column prop="testTime" label="测量时间" width="100">
        <template slot-scope="scope">
          <el-date-picker
      ...
      <u-table-column prop="testValue" label="测量值" width="100">
        <template slot-scope="scope">
          <el-input v-model="scope.row.testValue" :disabled="isCheck"/>
        </template>
      </u-table-column>
      ...
      <u-table-column prop="spcGroup" label="群组" width="100">
        <template slot-scope="scope">
          <el-input v-model="scope.row.spcGroup" :disabled="isCheck"/>
        </template>
      </u-table-column>
      <u-table-column prop="spcFileName" label="档案" width="100">
        <template slot-scope="scope">
          <el-input v-model="scope.row.spcFileName" :disabled="isCheck"/>
        </template>
      </u-table-column>
      <u-table-column prop="spcName" label="管制项目名称" width="100">
        <template slot-scope="scope">
          <el-input v-model="scope.row.spcName" :disabled="isCheck"/>
        </template>
      </u-table-column>
      ...
      <template v-for="field in this.dynamicList">
        <u-table-column :label="field.fieldNameDesc" :prop="field.fieldName" width="100">
          <template slot-scope="scope">
            <el-input v-if="field.htmlType === 'input'" v-model="scope.row[field.fieldName]" clearable ></el-input>
            <el-select v-else-if="field.htmlType === 'select'" v-model="scope.row[field.fieldName]"  clearable filterable  placeholder="请选择">
